...
export const demoArticles: DemoArticle[] = [
  {
    ...
    premium: [
      ...
      'The next step is the downstream user operation. In mock mode, the app uses an in-memory Zephr client to show the expected behavior: find by external subject, fall back to email when needed, create when no match exists, and otherwise update only the safe profile fields that are intended to move with the upstream identity. In real mode, the same behavior stays intact, but the methods in the Zephr client become the tenant-specific integration seam.',
      'That separation is deliberate. One of the biggest problems in integration prototypes is that they bake uncertain vendor assumptions directly into route handlers or UI pages. This project avoids that by keeping the uncertain Zephr API contract in one place. The rest of the application can remain stable while you decide which real Zephr surface to use for user provisioning and session creation.',
      'Session synchronization is the next piece that makes the experience feel coherent. If the external SAML login succeeds but the site-side Zephr session does not exist, the user may be authenticated according to the bridge but still look anonymous to the client-side Zephr runtime. That mismatch produces exactly the kind of confusing experience teams want to avoid: successful login followed by a wall that still behaves as if nothing happened.',
      'The bridge solves that by creating a downstream Zephr-authenticated session immediately after the user upsert step. In the mock implementation, this is represented by a local blaize_session cookie value and a structured success message. In a real tenant integration, the same step would need to exchange the bridged identity for whatever Zephr-recognized session or token mechanism your tenant supports.',
